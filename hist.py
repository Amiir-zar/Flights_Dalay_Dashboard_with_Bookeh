...
def hist_tab(data):
    ...
    def md(s_data, rs=-60, re=120, bin=10):
        d = pd.DataFrame(columns=[
                         'proportion', 'left', 'right', 'f_proportoin', 'f_interval', 'name', 'color'])
        r = re - rs
        for i, r_data in enumerate(s_data):
            subset = data[data['name'] == r_data]
            arr_hist, edge = np.histogram(
                subset['arr_delay'], bins=int(r / bin), range=(rs, re))
            arr_df = pd.DataFrame({
                'proportion': arr_hist / np.sum(arr_hist), 'left': edge[:-1], 'right': edge[1:]
            })
            arr_df['f_proportoin'] = ['%0.5f' %
                                      p for p in arr_df['proportion']]
    ...
    def mp(s_data):
        p = figure(plot_width=700, plot_height=700,
                   title='Delay Distribution', x_axis_label="Delay(Minutes)", y_axis_label="Proportion")
        p.quad(source=s_data, bottom=0, top='proportion', left='left',
               right='right', color='color', fill_alpha=0.7, legend='name')
        return p

    def update(attr, old, new):
        air_lines_checked = [chbox.labels[i] for i in chbox.active]
        ds = md(air_lines_checked,
                range_slider.value[0], range_slider.value[1], slider.value)
    ...
    slider = Slider(start=1, end=30, step=1, value=5,
                    title='Histogram bins : ')
    slider.on_change('value', update)

    range_slider = RangeSlider(
        start=-60, end=180, value=(-60, 120), step=5, title='Delay span:')
    ...
    l = row(w, p)
    tab = Panel(child=l, title='Airlines')
    return tab
